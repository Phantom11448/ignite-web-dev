...
Local-only test suite for `bizcheck/firestore.rules`' subscription/trial
access gating (`businessDoc`, `isInTrial`, `hasActiveAccess`). This folder
is **not** part of the deployed site — it's excluded from the Netlify
deploy via `.netlifyignore` at the repo root.
...
Every test should pass. If one fails, the failure message tells you
exactly which access check (which business age, which collection, which
role) didn't behave as expected — that's a real bug in `firestore.rules`
to go fix, not a flaky test.
